...
```{r}
folk <- get_playlist_audio_features("", "0xQcKjXspCi2IDxxKWX2ty")
...
dens_energy <- ggplot(indie, aes(x = energy, color = category)) + geom_histogram() + facet_wrap(~category) + 
...
```{r}
slow_hands <-
  get_tidy_audio_analysis("3qel23hQWhHT3qGewBQ0pE") |>
  select(segments) |>
  unnest(segments) |>
  select(start, duration, pitches)
...
slow_hands_plot <- slow_hands |>
  ...
  geom_tile() +
  labs(x = "Time (s)", y = NULL, fill = "Magnitude", title = "Chromagram of \"Slow Hands\"\nby Interpol") +
  theme_minimal() +
  scale_fill_viridis_c()

grid.arrange(j_lullaby_plot, slow_hands_plot, ncol=2)
```
