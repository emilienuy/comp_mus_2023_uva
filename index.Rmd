...
There is an ongoing debate on whether indie music can be considered a genre. Indie is short for “independent” and refers to music released by an artist independently rather than with a record label. Some bands that are considered to be indie, such as Arcade Fire, however, work with a record label. This begs the question: What makes indie music indie? Some argue that indie music has a specific sound, but what are its defining characteristics that allows Spotify to create “Indie” playlists? 

I hope to investigate this question with my corpus containing songs from three different indie sub-genre playlists created by Spotify: “Indie Rock Mix,” “Indie Pop Mix,” and “Indie Folk Mix.” You can scroll through the songs in each playlist displayed on the right. Comparing songs from these different sub-genres will clarify what characteristics of a song Spotify considers to be indie. The main limitation of my investigation is that the playlists only contain fifty songs each, and, since the sample is so small, these songs may not reflect the indie genre/sub-genres as a whole. 

Wikipedia describes indie folk as "hybridizing the acoustic guitar melodies of traditional folk music with contemporary instrumentation." Typical indie folk songs include "Naked As We Came" and "Flightess Bird, American Mouth" by Iron & Wine, and "White Winter Hymnal" by Fleet Foxes. 

Indie rock developed from rock music, but is known to have a softer sound to it compared to rock. Typical indie rock songs include "A-Punk" by Vampire Weekend and "You Only Live Once" by The Strokes.

Wikipedia describes indie pop as "a music genre and subculture that combines guitar pop with DIY ethic in opposition to the style and tone of mainstream pop music." Typical indie pop songs include "cielings" by Lizzy McAlpine and "Oh Caroline" by The 1975.

Each indie sub-genre has stemmed off of another genre (folk, rock, and pop), but they all can be grouped together to have an "indie" sound. I will investigate in which ways the three differ as well as how they coincide. I would hypothesize indie pop to have the most popular music because of the name, indie rock to be the most energetic, and indie folk to be the most acoustic.

Column {data-width=300}
-------------------
<iframe src="https://open.spotify.com/embed/playlist/5dMoFfrgm8aOOkneQjt6LT?utm_source=generator" height="100%" width="100%" style="border: none" allowfullscreen allow="autoplay" data-external="1">
</iframe>
<iframe src="https://open.spotify.com/embed/playlist/3HDdTtyB76aLDc4wpGFCX4?utm_source=generator" height="100%" width="100%" style="border: none" allowfullscreen allow="autoplay" data-external="1">
</iframe>
<iframe src="https://open.spotify.com/embed/playlist/6P82OCSEpo1VrZ4TZ2W5jK?utm_source=generator" height="100%" width="100%" style="border: none" allowfullscreen allow="autoplay" data-external="1">
</iframe>
...
### Does Indie Pop Live Up To Its Name? {data-commentary-width=500}
``` {r}
pop <- indie %>%
  group_by(category) %>%
  summarise(pop_mean = mean(track.popularity), dance_mean = mean(danceability))

dance_pop <- indie |>                    # Start with awards.
  mutate(
    mode = ifelse(mode == 0, "Minor", "Major")
  ) |> ggplot(aes(x = danceability, y = track.popularity, color=mode, label = track.name)) +
  geom_point() + facet_wrap(~category) + labs(x = "Danceability", y = "Popularity", color = "Mode") +
  scale_color_manual(values = c("#00AFBB", "#E7B800")) + 
  geom_hline(pop, mapping = aes(yintercept = pop_mean),col="purple", size=0.3) +
  geom_vline(pop, mapping = aes(xintercept = dance_mean),col="blue", size=0.2)

ggplotly(dance_pop)
```

***
This graph shows the relationship between a track’s popularity and danceability. Spotify’s danceability audio feature measures how suitable a track is for dancing based on tempo, rhythm, and beat. 0 means not danceable and 1 means very danceable. Track popularity is a measure between 0 and 100, with 100 being most popular and 0 being least popular. Track popularity is measured based on how many times the track has been played. We can observe that there is no clear relation between popularity and danceability; a popular song can be either danceable or not, and vice versa. The purple horizontal line represents the mean track popularity of each playlist, and the blue vertical line represents the mean danceability.

Indie pop, based on its name, should have the songs with the most popularity. It does indeed have the most popular song in the entire playlist, “ceilings” by Lizzy McAlpine, with a popularity of 91. It also, however, has the song with the least popularity, “Scandal” by Emerald. Surprisingly, the “Indie Pop” playlist has the lowest popularity mean of 52. “Indie Folk” has a mean of 57, and “Indie Rock” has the highest mean of 59. Hence, Spotify’s “Indie Pop” playlist does not have the most popular songs in contrast to its name. All three playlists have the majority of their tracks within a popularity measure range of 40 and 80. The three playlists therefore do not differ too much in popularity.

Although indie pop does not win the popularity contest, it does have the highest average danceability score of 0.7. Indie folk and rock have similar averages of 0.5.

We can also observe the different modes, major or minor, used in each track. Both Indie Folk and Pop have the majority of their tracks in a major key, while Indie Rock has about half of its tracks in a minor key.

In summary, track popularity is quite similar across the three playlists, Indie Pop is the most danceable playlist, and Indie Rock has songs played in more minor keys than the other two sub-genres.
...
### Typical Indie Folk: Repeating Chroma & Simple Timbre {data-commentary-width=500}
``` {r}
flightless <-
...
### Typical Indie Pop: Simple Chroma & Simple Timbre {data-commentary-width=500}
``` {r}
cielings <-
...
grid.arrange(top="\"Cielings\" by Lizzy McAlpine", cielings_chroma, cielings_timbre, ncol=2)
```

***
“ceilings” by Lizzy McAlpine is the most popular song in the “Indie Pop” playlist. She released self-released her first EP “Indigo” in 2018. She released “ceilings” in 2022 under the AWAL label. AWAL, or Artists Without A Label, is a British company that serves as an alternative to the traditional music label deal, offering artists and independent labels without them giving up ownership or control. Hence, although “ceilings” was released under a label company, it could still be considered “independent.” 

“ceilings” has the following structure: verse 1, chorus, verse 2, chorus, outro. The first 20 seconds of the song consists of an acoustic guitar solo, and then McAlpine joins in with vocals. We can see a slight change in timbre at that time, marking the beginning of the first verse. The chorus starts at 60 seconds, and we see a thin yellow line in the timbre matrix at this time. This could be due to her change in her use of breath. When she starts singing the chorus, McAlpine sings in a more breathy, airy tone. 

At 90 seconds, which marks the beginning of verse 2, half notes played by a base and faint percussion sounds are added to the background of the vocals. This could explain the sudden bright change we can observe in the chroma matrix. A violin than comes in just before 100s, which is what the thin bright line in the timbre matrix could be showing. 

At 120 seconds, Lizzy McAlpine sings the second chorus in a higher pitch. This is represented by a bright line in the chroma matrix. Just before 150 seconds, she begins the outro in yet another higher pitch, which is also represented in the chroma matrix.

In summary, the structure of Lizzy McAlpine’s “ceilings” can be clearly seen through analyzing both chroma-based and timbre-based self-similarity matrices.
...
### Indie Keys
```{r}
key_histogram <- ggplot(indie, aes(x=key, color=category, fill=category)) + 
  geom_histogram(binwidth = 0.5, alpha=0.6) + 
  scale_x_continuous(breaks = c(0:11),labels=c("C","C#","D","D#","E","F","F#","G","G#","A","A#","B")) +
  labs(y = "Count", x="Key", title = "Comparing the Distribution of Key Between Indie Subgenres")
ggplotly(key_histogram)
```

***
This histogram shows the distribution of keys in each indie subgenre playlist. The most common used keys in Indie Pop are C and C#. This is to be expected, as C major is the most common key used in pop music. Most of the keys are evenly distributed across Indie Pop as a whole, so we can conclude that Indie Pop does not have rules for what key should be used. Indie Folk also uses most of the keys equally with 2 keys that are in minority: G# and A#. Indie Rock also has most of its songs evenly distributed amongst the different keys, with three keys in minority: D#, G#, and A#. Overall, C is the most common key used in all three genres, followed by D and F. Since the keys used in each sub-genre are widespread, a song’s key cannot help us conclude whether or not it belongs to the indie genre.
...
in_love |>
  ...
  scale_fill_viridis_c(guide = "none") +
  theme_minimal() +
  labs(x = "Time (s)", y = "", title = "Chordogram of \"I'm In Love With You\" by The 1975")
```

***
The 1975 are a popular indie pop band, signed under the Dirty Hit label, so they are not an independent band, but still considered to be indie. The band consists of a lead vocalist and guitarist Matthew Healy, a lead guitarist Adam Hann, a bassist Ross MacDonald, and a drummer George Daniel. The chordogram on the left is of their song “I’m In Love With You.” It shows two major chords used throughout the song: A7 and D7. 

The first 15 seconds begin with a guitar solo, which we can see showing strong chord similarity of A7 and D7 in the chordogram. After this, the vocals come in, and the fainter color shows less strong similarity to chord templates, which could be due to the vocals overlapping with the guitar chords. At around 60s, the chorus begins with a repetition of the lyrics “I’m in love with you, I- I- I- I- I” eight times. This chorus is clearly in a D7 chord as shown on the chordogram between 60 and 100 seconds. Then the song goes back to an A7 chord. The rest of the song is focused on the D7 chord. 


### "A-Punk" by Vampire Weekend {data-commentary-width=500}
``` {r}
a_punk_ <-
  get_tidy_audio_analysis("3AydAydLzyyZutA0375XIz") |>
  compmus_align(sections, segments) |>
  select(sections) |>
  unnest(sections) |>
  mutate(
...
a_punk_ |>
  ...
  scale_fill_viridis_c(guide = "none") +
  theme_minimal() +
  labs(x = "Time (s)", y = "", title = "Chordogram of \"A-Punk\" by The Vampire Weekend")
```

***
As shown by the chroma-based self-similarity matrix earlier, the chroma in “A-Punk” by The Vampire Weekend is not varied. The chordogram shows the consistent use of a D7 chord, which is simple to play on the guitar. This further confirms that the chroma is simple in Indie Rock.
...
This question is very difficult to answer. There are very clear differences between the three indie sub-genres. These differences are highlighted below:

***Track-Level Features - Energy, Danceability, Popularity, & Valence***

My analysis has shown that indie rock is the most energetic and loud (the features have a positive correlation) sub-genre, followed by indie pop and then indie folk. Indie Pop is the most danceable playlist, yet the least popular based on Spotify's audio features. This is surprising as indie pop has popularity in its name. Lastly, indie folk is the most sad playlist, which makes sense because folk music can have a nostalgic feel to it.

***Instrumentation***

Indie rock uses electric guitars, bass, and drums, and therefore has scores higher on the "brightness" and "loudness" timbre measures. Indie folk uses acoustic instruments, mainly the acoustic guitar, which explains why it has the highest acoustic measures. Indie pop does not have typical instruments; indie pop songs can range from acoustic to loud and bright. Hence, the indie genre follows the verse & chorus structure in general.

***Structure***

Indie rock and pop songs have very similar song structures with a typical verses and choruses. Indie folk songs have much more repetition in lyrics and melodies than indie rock and pop, but they also tend to follow a verse and chorus structure. 

***Chroma***

Indie rock has an equal number of songs in major and minor keys, while the other two sub-genres have the majority of their songs in a major key. Indie folk has the most variety in pitch, which compensates for its instrumentation which does not have much variety. Indie rock and pop songs mostly stick to the same pitch. The indie genre overall can have songs with any key. The key in which a song is played cannot help indicate whether it is indie.

***Tempo***

The three indie sub-genres all have an average tempo of 120BPM. Indie rock has the songs with the fastest tempo, while indie folk is the slowest. Furthermore, overall, indie songs stick to a chosen tempo and do not deviate from this throughout the song.


Overall, indie can be many things. An indie song can have any measure of energy, valence, tempo, timbre, danceability, etc. An indie song usually has a structure consisting of verses and choruses and a consistent tempo. But other than this, indie songs can sound very different from each other. The sub-genres indie rock and folk, however, are easy to group and distinguish. Indie pop has much more variety and it harder to categorize, as shown through the clustering and classification. To really understand what makes a song indie, I would investigate differences between the original genres that the indie-genres stemmed from. For example, I would compare indie rock to rock, indie folk to folk, and indie pop to pop. This investigation has shed light on how much variety the indie genre has. Its sub-genres have clear musical differences, but my findings have still not answered the question "what does indie sound like as a whole?"
